...
function clockUpdateTime(e,t,z){
const TZ=z||"Asia/Shanghai";
...
<div class="clock-row"><span id="card-clock-time"></span></div>
<div class="clock-row">
...
const P=(e,t)=>("0".repeat(t)+e).slice(-t);

function U(){
const d=new Date;
const x=new Date(d.toLocaleString("en-US",{timeZone:TZ,hour12:false}));
document.getElementById("card-clock-time").innerHTML=
P(x.getHours(),2)+":"+P(x.getMinutes(),2)+":"+P(x.getSeconds(),2);
document.getElementById("card-clock-clockdate").innerHTML=
`${x.getFullYear()}-${P(x.getMonth()+1,2)}-${P(x.getDate(),2)} ${W[x.getDay()]}`;
document.getElementById("card-clock-dackorlight").innerHTML=
x.getHours()>12?" P M":" A M"
}
setInterval(U,1e3);U()
}

function getIpAndWeather(d){
const F=n=>{
fetch(`${qweather_api_host}/geo/v2/city/lookup?location=${encodeURIComponent(n)}&key=${qweather_key}`)
.then(e=>e.json())
.then(e=>{
const l=e.location&&e.location[0];
if(!l)return;
fetch(`${qweather_api_host}/v7/weather/now?location=${l.id}&key=${qweather_key}`)
.then(e=>e.json())
.then(e=>clockUpdateTime(e,n,l.tz))
})
};
d&&d.trim()?F(d):
fetch(`https://restapi.amap.com/v3/ip?key=${gaud_map_key}`)
.then(e=>e.json())
.then(e=>F(e.city||e.province||"未知城市"))
}

(function wait(){
if(typeof qweather_key!=="undefined"&&
